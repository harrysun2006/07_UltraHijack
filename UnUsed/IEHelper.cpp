...
IHTMLFormElement* GetForm(IHTMLDocument2 *pIHTMLDocument2, LPCSTR lpsz)
{
	...
	for(long i = 0; i < nFormCount; i++)
	{
		...
		spFormElement->get_name(&bstrName);
		name = bstrName ? OLE2CT(bstrName) : _T("");
		if (name.Compare(lpsz) == 0) return spFormElement;
	}
	return NULL;
}
...
{
	if(!pIHTMLFormElement) return NULL;
	...
	long nElemCount = 0;
	hr = pIHTMLFormElement->get_length(&nElemCount);
	if (FAILED(hr))	return NULL;

	for(long j = 0; j < nElemCount; j++)
	{
		hr = pIHTMLFormElement->item(CComVariant(j), CComVariant(), &pDisp);
		if (FAILED(hr)) continue;

		CComQIPtr<IHTMLElement> spInputElement = pDisp;
		// pDisp->Release();

		hr = spInputElement->get_id(&bstrName);
		if(FAILED(hr)) continue;
		name = bstrName ? OLE2CT(bstrName) : _T("");
		if (name.Compare(lpsz) == 0) return spInputElement;
	}
